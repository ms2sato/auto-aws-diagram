...
export class SvgGenerator {
  ...
  generateSvg(
    ...
    // SVGインスタンスの作成
    const svg = newInstance().width(canvasWidth).height(canvasHeight)
  ...
  private calculateTreeLayout(): void {
    ...
    for (let level = 0; level <= this.maxLevel; level++) {
      ...
      nodes.forEach((node) => {
        const factor =
          typeSpacingFactor[node.resource.type] || typeSpacingFactor.default
    ...
    for (let level = 0; level <= this.maxLevel; level++) {
      ...
      if (nodesCount === 1) {
        ...
        nodes.forEach((node) => {
          ...
          const spacing =
            typeHorizontalSpacing[node.resource.type] || horizontalSpacing
  ...
  private drawResources(svg: any, resources: Resource[]): void {
    ...
    for (const resource of resources) {
      ...
      const factor =
        typeSpacingFactor[resource.type] || typeSpacingFactor.default
  ...
  private drawRelationConnections(svg: any, connections: Connection[]): void {
    ...
    const relationConnections = connections.filter(
      (conn) => conn.type !== 'belongs_to'
    )
  ...
  private calculateRequiredCanvasSize(): { width: number; height: number } {
    ...
    const xOffset =
      minX < this.options.padding ? this.options.padding - minX : 0
